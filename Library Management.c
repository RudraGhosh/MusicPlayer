    ...
    do
    {
        ...
        if(p==0)
        {
            ...
            while(fscanf(fp,"%s %s %s %s %s %s",book_iname,book_iauth,book_iid,stu_iname,stu_iid,is_date)!=EOF)
            {
                ...
                {
                    fprintf(fp3,"%s %s %s %s %s %s",book_iname,book_iauth,book_iid,stu_iname,stu_iid,is_date);
                }
            }
            fclose(fp);
            fclose(fp3);
        ...
        fclose(fp);
        fp=fopen("bookistemo.txt","r");
        while(fscanf(fp,"%s %s %s %s %s %s",book_iname,book_iauth,book_iid,stu_iname,stu_iid,is_date)!=EOF)
        {
            fp1=fopen("bookissuerecord.txt","a");
            fprintf(fp1,"%s %s %s %s %s %s",book_iname,book_iauth,book_iid,stu_iname,stu_iid,is_date);
            fclose(fp1);
        }
        fclose(fp);
        fp=fopen("booktemp.txt","w");
